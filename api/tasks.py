...
def uploadAllImages():
    ...
    for user in users:
        ...
        document_id = [
            x for x in documents if x["name"] == CAPTRICITY_TEMPLATE_NAME
        ].pop()["id"]
        ...
        for image in images:
            ...
            try:
                ...
                imageObject = ApiBatchImage.objects.create(
                    batch=batchObject, image=image
                )
                imageObject.save()
            except:
                pass
...
def mailUser():
    ...
    for batch in batches:
        ...
        if data.count() == 0:
            ...
            data.save()
            send_mail(
                "Captricity data completed.",
                "Your data is completed. The link for the data is http://localhost:8000/api/data/"
                + str(batch.id)
                + "/",
