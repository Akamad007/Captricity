...
import captools.api
from Captricity.settings import (
    CAPTRICITY_API_TOKEN_SUBMIT,
    CAPTRICITY_API_TOKEN,
    BATCH_NAME as STORED_BATCH_NAME,
    CAPTRICITY_TEMPLATE_NAME,
)
...
def add(request, id):
    ...
    document_id = [x for x in documents if x["name"] == CAPTRICITY_TEMPLATE_NAME].pop()[
        "id"
    ]
    ...
    try:
        ...
        messages.success(
            request, "The image was successfully added to batch " + batchObject.name
        )
        ...
        messages.success(
            request,
            "There was an Error in the batch " + batchObject.name + " " + str(e),
        )
...
def get_random_batch_name(STORED_BATCH_NAME):
    BATCH_NAME = STORED_BATCH_NAME + "".join(
...
def addall(request):
    ...
    try:
        ...
        messages.success(
            request,
            "All images were successfully added to batch "
            + batchObject.name
            + " total images "
            + str(images.count()),
        )
...
def view_all_batches(request):
    success_batches = ApiBatch.objects.filter(user=request.user, success=True).order_by(
        "-dateTime"
    )
    failure_batches = ApiBatch.objects.filter(
        user=request.user, success=False
    ).order_by("-dateTime")
    return render(
        request,
        "api/batches.html",
        {"success_batches": success_batches, "failure_batches": failure_batches},
    )
...
def data(request, id):
    ...
    try:
        ...
        return HttpResponseRedirect("/home/")
    data = data.text.splitlines()    for n, d in enumerate(data):
        data[n] = d.split(",")
