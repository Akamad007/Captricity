...
class CallbackHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.wfile.write(
            (
                "<html><head><title>Captricity API Token</title></head><body>%s</body></html>"
                % body
            ).encode()
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Authenticate user and obtain an API token to access the Captricity API. Assumes that you have already registered an application on Captricity.\nSee https://shreddr.captricity.com/developer/#register for more info."
    )
    parser.add_argument(
        "--endpoint",
        required=True,
    ...
    parser.add_argument(
        "--port", default=8765, help="Port number for listening to the callback request"
    )
