...
class Client(object):
    ...
    def __init__(
        self,
        api_token,
        endpoint="https://shreddr.captricity.com/api/backbone/schema",
        version=None,
    ):
        """
        ...
        for resource in self.schema["resources"]:
            if "GET" in resource["allowed_request_methods"]:
                read_callable = _generate_read_callable(
                    resource["name"],
                ...
                setattr(
                    self, read_callable.__name__, types.MethodType(read_callable, self)
                )
            if "PUT" in resource["allowed_request_methods"]:
                update_callable = _generate_update_callable(
                    resource["name"],
                    resource["display_name"],
                    resource["arguments"],
                    resource["regex"],
                ...
                setattr(
                    self,
                    update_callable.__name__,
                    types.MethodType(update_callable, self),
                )
            if "POST" in resource["allowed_request_methods"]:
                create_callable = _generate_create_callable(
                    resource["name"],
                    resource["display_name"],
                    resource["arguments"],
                    resource["regex"],
                    resource["doc"],
                    resource["supported"],
                    resource["post_syntaxes"],
                    resource["is_action"],
                )
                setattr(
                    self,
                    create_callable.__name__,
                    types.MethodType(create_callable, self),
                )
            if "DELETE" in resource["allowed_request_methods"]:
                delete_callable = _generate_delete_callable(
                    resource["name"],
                    resource["display_name"],
                    resource["arguments"],
                    resource["regex"],
                ...
                setattr(
                    self,
                    delete_callable.__name__,
                    types.MethodType(delete_callable, self),
                )
    ...
    def print_help(self):
        ...
        resources = groupby(
            sorted(
                filter(
                    lambda x: (
                        hasattr(x, "is_api_call")
                        and x.is_api_call
                        and x.is_supported_api
                    ),
            ...
            for r in resource_methods:
                ...
                if r._put_or_post_args:
                    put_or_post_args = [
                        arg["name"]
                        for arg in reduce(
                            lambda x, y: x + y, r._put_or_post_args.values()
                        )
                    ]
    ...
    def _put_or_post_multipart(self, method, url, data):
        ...
        if resp.status not in [200, 202]:
            raise IOError(
                "Response to %s to URL %s was status code %s: %s"
                % (method, url, resp.status, resp.read())
            )
        return json.loads(resp.read())
...
def _generate_update_callable(
    name, display_name, arguments, regex, doc, supported, put_arguments
):
    ...
    def f(self, *args, **kwargs):
        for key, value in args[-1].items():
            if isinstance(value, IOBase):
                return self._put_or_post_multipart(
                    "PUT", self._generate_url(regex, args[:-1]), args[-1]
                )
        return self._put_or_post_json(
            "PUT", self._generate_url(regex, args[:-1]), args[-1]
        )
...
def _generate_create_callable(
    name, display_name, arguments, regex, doc, supported, post_arguments, is_action
):
    ...
    def f(self, *args, **kwargs):
        for key, value in args[-1].items():
            if isinstance(value, IOBase):
                return self._put_or_post_multipart(
                    "POST", self._generate_url(regex, args[:-1]), args[-1]
                )
        return self._put_or_post_json(
            "POST", self._generate_url(regex, args[:-1]), args[-1]
        )
