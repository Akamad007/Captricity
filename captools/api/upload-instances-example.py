...
def main():
    ...
    parser.add_argument(
        "--endpoint",
        required=True,
        help="the URL to the API definition resource, like https://shreddr.captricity.com/api/backbone/schema",
    )
    parser.add_argument(
        "--apitoken",
        required=True,
    ...
    parser.add_argument(
        "--job", required=True, type=int, help="the ID number for the job resource"
    )
    parser.add_argument(
        "image",
    ...
    for i in range(int(math.ceil(len(args.image) / float(page_count)))):
        ...
        for page_number, image in enumerate(
            args.image[i * page_count : (i * page_count) + page_count]
        ):
            print("\t", image.name)
            client.create_iset_instance(
                iset["id"], page_number, {"image": image, "image_name": image.name}
            )
